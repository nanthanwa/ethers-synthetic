...
contract SimpleLending {
    ...
    function createKey(bytes memory a, bytes memory b)
        public
        pure
    ...
    function addSupportedAssets(ERC20Interface asset, bytes memory symbol)
        public
    {
    ...
    function getRate(ERC20Interface asset1, ERC20Interface asset2)
        internal
        returns (uint256)
    {
        ...
        (bytes32 rawRate, , QueryInterface.QueryStatus status) =
            oracle.query.value(oracle.queryPrice())(key);
    ...
    function liquidate(ERC20Interface asset) public {
        ...
        uint256 payOffassetBackedAmount =
            (ln.payOffAmount * exchangeRate) / denominator;

        require(
            ln.assetBackedAmount <
                (payOffassetBackedAmount * liquidationRatio) / denominator
        );
