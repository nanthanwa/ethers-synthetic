...
describe('Liquidation Testing', async () => {
    ...
    it('Can liquidate $dTSLA', async () => {
        ...
        if (allowance.lte(dollyAmount)) {
            actual = await dolly.connect(deployer).approve(synthetic.address, ethers.constants.MaxUint256);
            assert.ok(actual.hash);
        }

        // console.log('doppleTSLA.address, syntheticAmount, dollyAmount', doppleTSLA.address, syntheticAmount, dollyAmount);
        ...
        if (allowance.lte(assetBackedAtRateAmount)) {
            console.log('allow more');
            actual = await dolly.connect(liquidator).approve(synthetic.address, ethers.constants.MaxUint256);
            assert.ok(actual.hash);
        }
